...
class ElGamal:
	...
	def generate_public_key(self):
		...
		"""
		return pow(self.gen,self.secret,self.prime)
	...
	def encrypt(self, m):
		...
		"""
		r = self.gen_random()
		c = pow(self.gen,r,self.prime)
		c2 = (m*pow(self.public,r,self.prime))% self.prime
		return (c,c2)
	...
	def decrypt(self, c):
		...
		"""
		s = pow(c[0],self.secret,self.prime)
		si = self.calc_inverse(s)
		m = (si* c[1]) % self.prime
		return m
...
def big_step_baby_step(w,g,n,verbose=False):
	...
	cons = pow(g,n_sqrt*(n-2),n)
	for j in range(n_sqrt):

		look = (w*pow(cons,j,n)) % n
		expected = table.get(look)
		if expected != None:
			v = j* n_sqrt +expected
			return v
...
secret = d_log(123,5,1333779268829507)[1]
...
print("Martin Luther King or Dr. Strangelove")
